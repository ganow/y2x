...
class Records {
  ...
  view(viewFmt) {
    ...
    } else if (typeof viewFmt === 'function') {
      records.viewFmt.setDefault(viewFmt)
    } else {
  ...
  sortBy(entry, reverse) {
    var records = new Records(this)

    if (typeof entry === 'undefined') {
      return records
    } else if (typeof entry === 'string') {
      const rev = ( (typeof reverse === 'boolean') && reverse ) ? -1 : 1
      records.sortFn = (lhs, rhs) => {
        if ( lhs[entry] < rhs[entry] ) { return -1 * rev }
        if ( lhs[entry] > rhs[entry] ) { return 1 * rev }
        return 0
      }
    } else if (typeof entry === 'function') {
      records.sortFn = entry
    }
    return records
  }

  render() {
    var data = this.data.filter((record) => this.contain(record))
    if (this.sortFn) {
...
function format(viewFmt, record) {
  ...
  if (typeof viewFmt === 'function') {

    return viewFmt(record)
